...
const GameShow = ({ gameShow, getGameShow, resetGameShow, gameSlug }) => {
  ...
  const renderPlatforms = () => {
    if (gameShow.platforms && gameShow.platforms.length) {
      const platforms = gameShow.platforms
        .filter(
          (p) =>
    ...
    <div>
      {loaded ? (
        ...
        >
          ...
          <Jumbotron
            className="game-show-jumbo mb-0"
            fluid
            style={{ backgroundImage: `url(${gameShow.background_image})` }}
          >
            <Container></Container>
          </Jumbotron>
          <Container>
            <Row className="justify-content-center mt-5">
              <Card
                className="col-11 col-md-12 p-0 mb-5"
                style={{ borderRadius: "20px" }}
              >
                <h1 className="py-4 px-2 mb-0 game-show-header text-center">
                  {gameShow.name}
                </h1>
                  ...
                  <div className="my-5">
                    ...
                    <Table striped hover>
                      <tbody>
                        ...
                        >
                          <td colSpan="2" className="pl-3 pl-md-4 pl-lg-5">
                            Release Date
                          </td>
                          <td className="text-center" colSpan="2">
                            {formatDate(gameShow.released)}
                          </td>
                        </tr>
                        <tr
                          style={{
                      ...
                      <tbody>
                        <tr className="text-center">
                          <td className="d-none d-md-table-cell">
                            {gameShow.reviews_count}
                          </td>
                          {gameShow.ratings.length >= 4 ? (
                            <td className="d-none d-sm-table-cell">
                              {gameShow.ratings[3].percent}%
                            </td>
                          ) : (
                ...
                <Row className="justify-content-center my-5">
                  <Link
                    ...
                    to={`/games/${gameShow.slug}/videos`}
                  >
                    <Button
                      className="p-3 youtube-link-btn"
                      variant="primary"
                      size="lg"
                      block
                    >
...
export default connect(mapStateToProps, { getGameShow, resetGameShow })(
  GameShow
);
